...
void PrintBootSectInfo(NTFS_BootRecord _check)
{
	...
	printf("=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*\n");
	printf("Инструкция по сборке, переход к загрузочному коду: 0x%X\n", _check.jumpCode);
	printf("Версия производителя: %s\n", _check.oemID);
	printf("Количество байт в секторе: %d\n", _check.bytesPerSector);
	printf("Количество секторов в кластере: %d\n", 0x01 << _check.sectorPerCluster);
	printf("Зарезервированные сектора: %d\n", _check.reservedSectors);
	printf("Медиа дескриптор: 0x%X\n", _check.mediaDiscr);
	printf("Количество секторов в дорожке: %d\n", _check.sectorperTrack);
	printf("Количество головок ЗУ: %d\n", _check.numOfHeaders);
	printf("Скрытые секторы: %d\n", _check.hiddenSectors);
	printf("Общее количество секторов: %dll\n", _check.totalSectors);
	printf("Номер логического кластера для файла $MFT: %dl\n", _check.lcnForSMFT);
	printf("Номер логического кластера для файла $MFTMirr: %dl\n", _check.lcnForSMFTMirr);
